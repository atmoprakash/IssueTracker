...
public class IssueDaoImpl implements IssueDao {
    ...
    String sql="update issue_tracker set issuename=?,servicename=?,issuedate=?,customername=?,createdby=? where id=?";
        JdbcTemplate jdbcTemplate=new JdbcTemplate(dataSource);
        java.util.Date dt = new java.util.Date();
        java.text.SimpleDateFormat sdf =new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        String updateTime = sdf.format(dt);
        jdbcTemplate.update(sql,new Object[]{user.getIssuename(),user.getServicename(),updateTime,user.getCustomername(),user.getCreatedby(),user.id});
    }

    @Override
    public void deleteData(Integer id) {
        String sql = "delete from issue_tracker where id=" + id;
        JdbcTemplate jdbcTemplate=new JdbcTemplate(dataSource);
    ...
    List<IssueModel> userList=new ArrayList<IssueModel>();
        String sql="select * from issue_tracker where id=" +id;
        JdbcTemplate jdbcTemplate=new JdbcTemplate(dataSource);
